<template>
    ...
    <form class="form-horizontal" action="***" method="post" enctype="multipart/form-data">
        <p class="title"><i class="fa fa-angle-left" @click="handelBack"></i> 发布需求</p>
        <div v-if="next" class="box-body">
            <div class="form-group">
                <label for="title">标题</label>
        
                <div>
                    <input type="text" name="title" id="title" placeholder="请填写商品标题" v-model="goodsMsg.title">
                </div>
            </div>
            <div class="form-group">
...
<script>
    ...
    export default {
        ...
        methods: {
            ...mapActions(['setBuyData']),
            handelNext(){
                if( !this.goodsMsg.title || !this.goodsMsg.price || !this.goodsMsg.msg || !this.goodsMsg.place){
                    // alert('请填完整信息再走！')
            ...
            handelPub(){
                if( 
                    ...
                    !this.goodsMsg.wechat ){
                        ...
                        this.Prompt = '求购发布成功，即将跳转到求购页'
                        this.popupVisible = true
                        console.log(this.goodsMsg)
                        let newGoods = {
                            address: this.goodsMsg.place,
                            title: this.goodsMsg.title,
                            detail: this.goodsMsg.msg,
                            buytime: new Date().toLocaleDateString().replace(/\//g,"-"),
                            price: this.goodsMsg.price,
                            tel: this.goodsMsg.phone,
                            qqnum: this.goodsMsg.QQ,
                            schooladdress: this.goodsMsg.school,
                            headpic: 'http123',
                            username: this.user
                        }

                        this.setBuyData(newGoods)
                        setTimeout(() =>{
